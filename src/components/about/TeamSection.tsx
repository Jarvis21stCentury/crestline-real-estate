...
export function TeamSection() {
  ...
  useGSAP(
    () => {
      ...
      const cards = gridRef.current.querySelectorAll<HTMLElement>("[data-team-card]");
        ...
        <div ref={gridRef} className="grid gap-8 md:grid-cols-3">
          {TEAM.map((member) => (
            ...
            >
              ...
              <h3 className="font-cormorant text-xl text-oxford">{member.name}</h3>
              ...
              <div className="mt-6 border-t border-mercury/30 pt-6">
                <ul className="space-y-2">
                  {member.credentials.map((cred) => (
                    <li key={cred} className="flex items-start gap-2 font-body text-xs text-oxford/50">
